...
- **Wire up small/medium/large load tests from the start.** A single dataset size was tested, which limits the scaling conclusions. A parameterized load-test script that runs across three data sizes would have shown whether the observed scaling behavior held at different volumes.
- **Capture CPU/memory during load tests.** `docker stats` snapshots during scaling would have given resource-utilization context for the throughput numbers — useful both for conclusions and for knowing when a replica is CPU-bound vs. I/O-bound.
