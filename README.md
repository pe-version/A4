...
This section synthesizes the A4 work into a compact narrative. The `results/` subdirectories contain detailed evidence; the summaries below are meant to be read top-to-bottom without having to open every sub-document.

### Scaling Results

Horizontal scaling was tested on the Go sensor service behind an Nginx round-robin load balancer, with all state shared through a single Postgres instance (stateless services, no session affinity required).

**Read performance (1 replica → 3 replicas, 1,000 requests at concurrency 50):**

| Endpoint | Req/sec gain | p50 latency reduction | p90 latency reduction |
|----------|--------------|----------------------|----------------------|
| `GET /sensors` (list) | +61% | -49% | -37% |
| `GET /sensors/:id` (point) | +30% | -2% | -17% |

The improvement is sub-linear (not 3×) because the shared Postgres database becomes the new bottleneck once the HTTP layer has capacity. Production linear scaling would require read replicas, PgBouncer, or sharding.

**Write performance — the ID race story:** The initial scaling run reported 77 req/sec on `POST /sensors` at 3 replicas, but with a **94.5% failure rate**. Investigation traced the failures to a read-modify-write race in the repositories: each `Create` ran `SELECT MAX(id) + 1` inside a transaction, and concurrent transactions raced to the same next ID. Replacing this with a Postgres `SEQUENCE` (`nextval()` is atomic and lock-free) dropped the failure rate to **0.7%** — an 18× real improvement in successful write throughput that was entirely masked by the race until the scaling test exposed it. This was the engineering highlight of the assignment: the scaling experiment didn't just measure — it *surfaced a correctness bug* that would have bitten in production.

**Dataset size progression.** Follow-up runs at small (100 req) and large (5,000 req) sizes on the 3-replica configuration showed throughput saturating around **95 req/sec** with p50 latency actually *improving* as the dataset grows (458 ms → 320 ms) — warmup cost amortizes over longer runs. Docker stats captured during the large run confirmed the Postgres-is-the-bottleneck hypothesis empirically: Postgres at **50% CPU** with all three Go replicas combined at only ~45% CPU, and memory well under limits everywhere. The Go layer has headroom; the database is the constraint.

Evidence: [results/scaling/ANALYSIS.md](results/scaling/ANALYSIS.md), [results/scaling/screenshots/three-healthy-replicas.png](results/scaling/screenshots/three-healthy-replicas.png).

### Failure Testing Highlights

Three chaos scenarios were executed, each with a prediction, execution, observation, and an implemented improvement.

**1. Kill a replica.** Dropped one of three Go sensor replicas while requests were flowing. Nginx's default passive failover was correct but slow — one request stalled 14.3 seconds before Nginx gave up on the dead upstream and retried. *Improvement:* tuned `proxy_connect_timeout 2s` + `proxy_next_upstream_tries 2` in the Nginx config, reducing worst-case failover to ~21ms.

**2. Stop the database.** Stopped the `sensor-db` Postgres container. Data endpoints correctly returned HTTP 500; the health endpoint stayed up (confirming service-level liveness vs. data-layer readiness). But the write path exposed a problem: it returned HTTP 400 with the raw DNS resolution error leaked to the client. *Improvement:* introduced a `ValidationError` type to distinguish client errors from infrastructure errors, so DB failures now return HTTP 503 with a sanitized `"Service temporarily unavailable"` message.

**3. Disconnect the queue.** Stopped RabbitMQ. The result demonstrated **autonomous self-healing**: the consumer's reconnect loop retried every 5 seconds and succeeded the moment the broker came back, with no service restart required. The Prometheus counter delta proved exactly one message was lost (fire-and-forget publish during the outage), validating the at-least-once guarantee for everything that made it onto the broker. *Finding:* the system is crash-tolerant by design — the only loss vector is documented and accepted.

Evidence: [results/chaos/ANALYSIS.md](results/chaos/ANALYSIS.md).

### Observability Learnings

Three pillars were implemented and exercised end-to-end:

- **Structured logs with correlation IDs.** Every HTTP request carries an `X-Correlation-ID` (generated or client-supplied) that appears in every log line for that request, making it possible to sequence a failure precisely.
- **Metrics.** The alert service exposes Prometheus-format counters (`events_received_total`, `events_processed_total`, `alerts_triggered_total`) plus a processing-duration gauge. These counters enabled the chaos test 3 finding above — quantifying data loss to exactly one message by comparing counter deltas across a broker outage.
- **Distributed tracing across async boundaries.** A `trace_id` generated by the sensor service propagates through the RabbitMQ payload into the alert service's logs. A single trace ID follows a sensor update through publish → consume → rule evaluation → alert trigger, crossing the async boundary that would normally break conventional call-graph tracing.

The most important observability *lesson* came from the DB outage test: the duration pattern in the logs (`90ms → 3ms → 3ms`) was what diagnosed the failure as connection-level rather than query-level. That's the kind of insight that's only possible with structured logs — a simple text dump wouldn't have made the pattern visible.

Evidence: [results/observability/ANALYSIS.md](results/observability/ANALYSIS.md), [results/observability/screenshots/](results/observability/screenshots/).

### What Worked Well

- **Postgres migration enabled everything else.** Moving the Go services off SQLite to Postgres unlocked horizontal scaling, made the chaos tests meaningful (a real shared database can be killed independently), and let the ID race surface as a measurable bug instead of a SQLite write-lock rathole.
- **Scaling test as a bug-surfacing tool.** The 94.5% write-failure rate wasn't a scaling problem — it was a correctness bug that only showed up under concurrent load. Catching it in the scaling experiment and fixing it with the canonical Postgres sequence pattern was a real engineering win.
- **Five security improvements, each with OWASP mapping.** Network isolation (A01), secrets management (A02/A05), secure headers (A05), constant-time Bearer comparison (A07), and pinned base image (A06). The rubric asked for two; doing five with explicit mapping demonstrated defense in depth.
- **Three chaos scenarios with the full predict → execute → observe → improve cycle.** The third scenario (queue disconnect) was particularly useful because it showed where the system self-heals *and* where it silently loses data, both quantified with telemetry.
- **ADR-004 as a deferred-decision artifact.** Graceful shutdown was evaluated, found unnecessary for this stack (Postgres ACID + RabbitMQ requeue + Nginx LB failover make abrupt termination safe), and the decision was captured with revisit triggers and an upgrade path sketch. That's a mature engineering artifact — capturing *why we chose not to do something* is often more valuable than capturing what we did.
- **Selecting an all-Go stack for A4.** When told I only had to implement one language, I chose Go. The goroutine-per-request model, low per-replica memory footprint, and predictable garbage collector were especially relevant under horizontal scaling — and Python's strengths weren't a meaningful fit for what A4 actually asked for.

### What I'd Change Next Time — and Why

- **Investigate common anti-patterns proactively, not reactively.** The ID race fix, the constant-time auth improvement, and the error-sanitization fix were all caught by reactive reviews — a scaling test, a SWEBoK pass, a chaos test — after the code was written. A proactive pass through SWEBoK construction principles, OWASP Top 10, and known concurrency patterns during design would have caught most of them before they shipped. Next time: a checklist pass at the end of each service's design review, not just after the fact.
- **Wire up small/medium/large load tests *from the start*.** The three-size progression was eventually captured (see the scaling analysis), but only after the 1-vs-3 replica comparison was already done. A parameterized script from the beginning would have made the whole scaling experiment a single run instead of a retrofit, and would have avoided the side-effect where the three dataset-size runs operated on a DB that had accumulated different amounts of data from prior testing.
- **Capture CPU/memory during load tests consistently, not just once.** `docker stats` was captured during the large run and confirmed the Postgres-is-the-bottleneck hypothesis, but it would have been more valuable to capture it for every single test run rather than as a late addition — it would have made resource utilization part of the scaling comparison from the start.
- **Implement graceful shutdown if this becomes a portfolio piece.** Documented in ADR-004 as a deferred decision with concrete revisit triggers; the current abrupt-exit behavior is safe but cosmetic logs aren't great.
- **Reconsider keeping the Python services as an A3 baseline.** They didn't contribute to the A4 story and added cognitive overhead (two language stacks to document, the fanout exchange still feeds them even though they're not horizontally scaled). A cleaner A4 submission might have archived them and started fresh with a pure Go stack.

<!-- Personal/voice additions welcome here — anything specific you'd change that isn't captured above. -->
